...
function buildAuditRow(user, currentTrades, rebuiltTrades) {
  ...
  return {
    ...
    rebuiltOpenCount: rebuilt.openCount,
    openCountDelta: rebuilt.openCount - current.openCount,
    currentClosedCount: current.closedCount,
    rebuiltClosedCount: rebuilt.closedCount,
    closedCountDelta: rebuilt.closedCount - current.closedCount,
    ...
    currentQuantity: round(current.quantity, 4),
    rebuiltQuantity: round(rebuilt.quantity, 4)
  };
}
...
async function main() {
  ...
  audited.sort((left, right) => {
    if (right.absPnlDelta !== left.absPnlDelta) return right.absPnlDelta - left.absPnlDelta;
    return Math.abs(right.tradeCountDelta) - Math.abs(left.tradeCountDelta);
  });

  const topRows = audited.slice(0, args.limit);

  console.log(JSON.stringify({
    scannedUsers: users.length,
    skippedNoTrades,
    flaggedUsers: audited.length,
    filters: {
      limit: args.limit,
