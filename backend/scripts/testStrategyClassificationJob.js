...
async function testStrategyClassificationJob() {
  console.log('🧪 Testing Strategy Classification Job Processing\n');
  ...
  try {
    ...
    console.log('✅ Strategy classification completed successfully');
    console.log('   Result:', result);

  } catch (error) {
    console.error('❌ Strategy classification failed:', error.message);
    console.error('   Stack:', error.stack);
  }

  // DON'T close the database pool since the main server is running
  console.log('\n🔄 Test completed (database pool left open for main server)');
}
