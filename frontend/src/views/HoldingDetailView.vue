<template>
  <div class="content-wrapper py-8">
    ...
    <div v-else-if="error" class="bg-red-50 dark:bg-red-900/20 rounded-lg p-6 text-center">
      ...
      <button @click="loadHolding" class="mt-4 btn-primary">Try Again</button>
    </div>
    ...
    <div v-else-if="holding">
      ...
      <div class="mb-6">
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-center">
            ...
            >
              <span class="text-xl font-bold text-gray-500">{{ holding.symbol.slice(0, 2) }}</span>
            </div>
            <div>
              <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ holding.symbol }}</h1>
              <p v-if="profile?.name" class="text-gray-500 dark:text-gray-400">{{ profile.name }}</p>
              <p v-if="holding.broker" class="text-sm text-gray-400 dark:text-gray-500">{{ holding.broker }}</p>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
          ...
          <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatNumber(holding.shares) }}</p>
        </div>

        <!-- Current Value -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
          <p class="text-sm text-gray-500 dark:text-gray-400">Current Value</p>
          <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatCurrency(holding.currentValue) }}</p>
          <p v-if="holding.currentPrice" class="text-xs text-gray-400">
        ...
        <!-- Cost Basis -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
          <p class="text-sm text-gray-500 dark:text-gray-400">Cost Basis</p>
          <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatCurrency(holding.costBasis) }}</p>
      ...
      <div>
              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="lot in lots" :key="lot.id">
                  ...
                  <td
                    ...
                    :class="lot.gainLoss >= 0 ? 'text-green-600' : 'text-red-600'"
                  >
                    {{ formatCurrency(lot.gainLoss) }}
                    <span class="text-xs ml-1">({{ lot.gainLossPercent.toFixed(1) }}%)</span>
                  </td>
                ...
                <tr v-if="lots.length === 0">
                  <td colspan="7" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                    No purchase lots recorded. Add shares to track your cost basis.
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm text-gray-500 dark:text-gray-400">Total Dividends Received</p>
                  <p class="text-xl font-bold text-green-600">{{ formatCurrency(totalDividends) }}</p>
                </div>
                <div class="text-right">
                  <p class="text-sm text-gray-500 dark:text-gray-400">Yield on Cost</p>
                  <p class="text-xl font-bold text-gray-900 dark:text-white">
                    {{ holding.costBasis > 0 ? ((totalDividends / holding.costBasis) * 100).toFixed(2) : 0 }}%
                  </p>
                </div>
              </div>
            </div>
              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="dividend in dividends" :key="dividend.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500 dark:text-gray-400">
                    {{ dividend.perShare ? formatCurrency(dividend.perShare) : '-' }}
                  </td>
        ...
        <div v-if="activeTab === 'analysis'">
          <div v-if="analysisLoading" class="flex items-center justify-center py-16">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600"></div>
            <span class="ml-3 text-gray-500">Loading analysis...</span>
          </div>
          ...
          <div v-else class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8 text-center">
            <p class="text-gray-500 dark:text-gray-400 mb-4">No 8 Pillars analysis available</p>
        ...
        <div v-if="activeTab === 'notes'">
          <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Position Notes</h3>
            <textarea
              v-model="notes"
              rows="6"
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-6">
          Are you sure you want to delete this holding? This will also remove all lots and dividend records.
        ...
        <div class="flex justify-end space-x-3">
          ...
          <button @click="deleteHolding" class="btn-primary bg-red-600 hover:bg-red-700">
            Delete
          </button>
        </div>
      </div>
...
async function loadHolding() {
  ...
  try {
    ...
    lots.value = lotsData.map(lot => {
      ...
      const gainLossPercent = lot.totalCost > 0 ? (gainLoss / lot.totalCost) * 100 : 0
    ...
    try {
      profile.value = await investmentsStore.getProfile(holding.value.symbol)
    } catch (e) {
...
async function loadAnalysis() {
  ...
  try {
    analysis.value = await investmentsStore.analyzeStock(holding.value.symbol)
  } catch (err) {
...
async function saveNotes() {
  ...
  try {
    await investmentsStore.updateHolding(holding.value.id, { notes: notes.value })
